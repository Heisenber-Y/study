--企业到银行账号
DROP TABLE  IF EXISTS DWD_GRAPHBASE.ACCOUNT_TO_ENTERPRISE_TMP1;
CREATE  TABLE DWD_GRAPHBASE.ACCOUNT_TO_ENTERPRISE_TMP1 AS (
select
 GUM.POLICYNO,
...
DROP TABLE  IF EXISTS DWD_GRAPHBASE.ACCOUNT_TO_ENTERPRISE_TMP2;
CREATE  TABLE DWD_GRAPHBASE.ACCOUNT_TO_ENTERPRISE_TMP2 AS (
SELECT 
CONCAT('P00000000',NVL(T1.PAYEENAME,'未知'),NVL(T1.IDENTIFYTYPEALL,'未知'),NVL(T1.IDENTIFYNUMBER,'未知')) AS ENTERPRISEID
...
,T1.COMPANYCODE
FROM 
DWD_GRAPHBASE.ACCOUNT_TO_ENTERPRISE_TMP1 T1
INNER JOIN 
...
--获取个人到银行账号的关系
DROP TABLE  IF EXISTS DWD_GRAPHBASE.ACCOUNT_TO_ENTERPRISE_${DATE_CONF};
CREATE  TABLE DWD_GRAPHBASE.ACCOUNT_TO_ENTERPRISE_${DATE_CONF} AS (
SELECT 
T1.ACCOUNTNOID  
,T2.REGISTNO --报案号
...
,'ACCOUNT_OF_ENTERPRISE_OWN' KIND
...
,T1.ENTERPRISEID
,T2.REGISTNO REGISTNO_1 --报案号
,'' AS ENTERPRISE_KIND_NAME
,T1.PAYEENAME AS ENTERPRISE_NAME
,T1.IDENTIFYTYPEALL ENTERPRISE_KIND_CODE
,T1.IDENTIFYNUMBER  ENTERPRISE_CODE
,'' AS ENTERPRISE_LEVEL
,T1.MOBILEPHONE AS PHONE
,'ACCOUNT_OF_ENTERPRISE_OWN' AS KIND_1
,T1.ETL_DATE ETL_DATE_1
,T1.COMPANYCODE COMPANYCODE_ENTERPRISE
,T1.COMPANY COMPANY_ENTERPRISE
...
,'ACCOUNT_OF_ENTERPRISE_OWN'  AS RELATIONSHIP
, T1.ACCOUNTNOID SRCID
, T1.ENTERPRISEID DESID
FROM 
DWD_GRAPHBASE.ACCOUNT_TO_ENTERPRISE_TMP2 T1
LEFT JOIN 
DWD_GRAPHBASE.FILTER_GCCLAIMMAIN T2
ON T1.POLICYNO=T2.POLICYNO
WHERE T1.ENTERPRISEID IS NOT NULL
